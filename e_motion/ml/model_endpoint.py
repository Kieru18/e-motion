...
def train(project_id, model_id):
    ...
    module = CocoDetectorModule(initialize_model(model_id))

    loader_train = initialize_coco_loader(project_id, model_params["batch_size"], True)

    trainer = L.Trainer(limit_train_batches=100, max_epochs=model_params["epochs"], accelerator="gpu")
    trainer.fit(model=module, train_dataloaders=loader_train)

    trainer.save_checkpoint(model_checkpoint_from_id(model_id))
    
    return evaluate_detector(module, loader_train.dataset)
...
def predict(project_id, model_id):
    ...
    module = CocoDetectorModule.load_from_checkpoint(checkpoint_path, model=model)
...
def evaluate_detector(model: CocoDetectorModule, dataset):
...
def initialize_model(model_id):
    model_params = load_model_params(model_id)
    return CocoDetectorResnet(model_params["num_classes"], model_params["iou_thresh"])
...
def load_model_params(model_id):
    path = os.path.join(os.curdir, "models", model_id, "params.json")
    json_data = None
